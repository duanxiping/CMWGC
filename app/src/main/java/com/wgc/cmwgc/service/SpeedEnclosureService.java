...
//    private final String address = "ws://192.168.3.86:39966";
    ...
    private String gpsTime = WiStormApi.getCurrentTime();
    private double lon;
    private double lat;
...
//        Log.d(TAG,isFirst + "    -- 设备did -- ： " + did);
        if(!did.equals("")){
            ...
            if (gpsAcc == 0){
                ...
                sendACCLocation();/*5分钟发送一次定位数据给后台*/
            }
...
/*----------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    int locationCounts;
    private void sendACCLocation(){
        locationCounts++;
        if(locationCounts == 5*60){/*每隔30秒发送一次定位数据给后台*/
        if (isConnected) {
            String msg = getACCJSON(msgId, WiStormApi.getCurrentTime()).toString();
            Log.e(TAG, "ACC上传时间： " + msg);
            sendMsg(msg);
            msgId++;
        }
            locationCounts = 0;
...
//    int retryCheckConnectCount;
    ...
    private void checkConnection(){
        connectCount++;
        if(connectCount==10){
            Log.i(TAG, isConnected + " -----  十秒检查链路 --------- " + msgId);
...
//        }
        ...
        Log.i(TAG, "检查中。。。。。。。。"+checkConnectResponseCount+"");
    ...
    private void checkSpeedAlert() {
        ...
        if (speedCount < 9){
            if(speedGPS < speedLimit){//小于80公里/H{}
                alerts = "[]";
                bufferAlert = alerts;
//                Log.e(TAG,speedCount+"没有超速，正常速度是 ------- " + speedGPS);
                speedCount=0;
            }
        }else{
...
/****************************************************************************************************/
     ...
     * @return
     */
    private Object getACCJSON(int msgId,String ACCtime){
        JSONObject jObject=new JSONObject();
        try {
            jObject.put("msgId", msgId);
            jObject.put("type", "GPS");
