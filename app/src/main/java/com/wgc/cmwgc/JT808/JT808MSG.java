...
//        Log.d("BeiDouService",HexStringUtils.toHexString(AA) + " --- 验证码  --　" + HexStringUtils.toHexString(checkCode));
     ...
     * @return
     */
    public byte[] getLocation(String did,int alert,int status,int accState ,int lon,int lat,int heigh,int speed,int mileage, int direction,String time,int flowId){
       /* 消息体 */
        ...
        byte[] gpsAcc = new byte[4];
        gpsAcc = bitOperator.integerTo4Bytes(accState);
        ...
        byte [] body = new byte[alertFlay.length+gpsStatus.length+gpsAcc.length+gpsLon.length+gpsLat.length+gpsHeigh.length+gpsSpeed.length+2+gpsMileage.length+gpsDirection.length+gpsTime.length];
        System.arraycopy(alertFlay, 0, body, 0, alertFlay.length);
        System.arraycopy(gpsStatus, 0, body, 4, gpsStatus.length);
        System.arraycopy(gpsAcc, 0, body, 4, gpsAcc.length);
